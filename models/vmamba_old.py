...
#     def __init__(self, patch_size=4, in_chans=1, num_classes=3, depths=[2, 2, 9, 2], 
...
class VSSM(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.layer(x)
        x = self.norm(x)
        x = x.permute(0, 3, 1, 2)  # Reorder dimensions to match Conv2d expected input
...
# if __name__ == "__main__":
    ...
    #     in_chans=3,
    #     num_classes=3,
    #     depths=[1, 1, 4, 1],  # Reduced depths
    #     dims=[12, 24, 48, 96],  # Reduced dimensions
    #     d_state=8,  # Reduced d_state
    #     drop_rate=0.1,  # Increased drop_rate for regularization
    #     attn_drop_rate=0.,  # Increased attn_drop_rate for attention dropout
    #     drop_path_rate=0.,  # Increased drop_path_rate for stochastic depth
    #     norm_layer=nn.LayerNorm,
    #     patch_norm=False,  # Turned off patch_norm for simplicity
    #     use_checkpoint=False,
    #     final_upsample="expand_first"
    # ).to('cuda')
    ...
    # int = torch.randn(4, 3, 64, 64).cuda()
    # out = model(int)
    # print(out.shape)
...
#         num_classes=3,
#         depths=[1, 1, 4, 1],  # Reduced depths
#         dims=[24, 48, 96, 192],  # Reduced dimensions
#         d_state=8,  # Reduced d_state
#         drop_rate=0.1,  # Increased drop_rate for regularization
#         attn_drop_rate=0.1,  # Increased attn_drop_rate for attention dropout
#         drop_path_rate=0.2,  # Increased drop_path_rate for stochastic depth
#         norm_layer=nn.LayerNorm,
#         patch_norm=False,  # Turned off patch_norm for simplicity
#         use_checkpoint=False,
#         final_upsample="expand_first"
